...
import { getTags, getNecessaryScripts } from "./tags";
import { removeExternalStyles } from './utils';
...
function registerRendererHook(options) {
  ...
  this.nuxt.hook('vue-renderer:ssr:templateParams', (params) => {
    ...
    params.HEAD = params.HEAD
      .replace(scriptPattern, v =>
        (v.includes('custom-element') || v.includes('application/ld+json')) ? v : ''
      )
...
function find(arr, key, val = undefined) {
